...
<script>
	function write_go(){
		...
		document.location.href = 'controller?type=N_WriteView';
	}
</script>
</head>
<body>
...
<table class="table">
	...
	<c:choose>
		...
		<c:otherwise>
			...
			<tr>
				<td>${vo.n_idx }</td>
				<td><a href="controller?type=N_View&n_idx=${vo.n_idx }&cPage=${pvo.nowPage}">${vo.title }</a></td>
				<td>${vo.code }</td>
				<td>${vo.regdate }</td>
				<td>${vo.views }</td>
			</tr>
	...
	<tfoot>
		<tr>
			<td colspan="6">
				<ol class="paging">
				<li><a href="controller?type=N_List&cPage=1">첫페이지</a></li>
				<c:choose><%--[이전으로]에 대한 사용여부처리 --%>
					...
					<c:otherwise>
						<li>
							<a href="controller?type=N_List&cPage=${pvo.beginPage - 1 }"><</a>
						</li>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					...
					<c:otherwise>
						<li>
							<a href="controller?type=N_List&cPage=${pageNo }">${pageNo }</a>
						</li>
					</c:otherwise>
				</c:choose>
				</c:forEach>
				...
				<c:choose><%--[다음으로]에 대한 사용여부처리 --%>
					...
					<c:otherwise>
						<li><a href="controller?type=N_List&cPage=${pvo.endPage + 1 }">></a></li>
					</c:otherwise>
				</c:choose>
				<li><a href="controller?type=N_List&cPage=${pvo.totalPage} ">끝페이지</a></li>
				</ol>
			</td>
		</tr>
	</tfoot>
